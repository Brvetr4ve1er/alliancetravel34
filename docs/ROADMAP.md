...
| `ebfaf0e` | **C.1 Ambient animation cull** | Removed 3 keyframes + consumers that competed with focal motion: `tilt-bob` (related-card icon wiggle — parent card already lifts/shadows on hover), `globe-aura` (14s breathing halo — will be replaced by single slow rotation in Phase E), `globe-hint-bob` (2.6s bob on a one-shot tooltip). Visual elements (icon, halo gradient, hint pill) all preserved as static. 26 → 23 keyframes. |
| *pending* | **C.3 Pause-off-screen plumbing** | Added `initPauseOffScreen()` IntersectionObserver in `enhance-pro.js` that toggles `.is-paused` on any ambient host (`.home-hero`, `.hero`, `#alliance-globe`, `[data-pause-off-screen]`) when it scrolls out of viewport (120px rootMargin slack). New CSS rule `.is-paused, .is-paused * { animation-play-state: paused !important; }` freezes every keyframe inside the subtree, recovering the frame budget the browser would otherwise spend ticking off-screen ambient loops. Per Performance Contract §0a item 2. C.2 (choreographing surviving twinkle/glow/drift loops) folded into Phase E since they live on the trip-page heroes that get redesigned there. |

Phase A.2 (spacing/radii/elevation geometric migration) and Phases D–J still pending.
