 ...
 *      only counts [data-counter] nodes (the homepage hero stats); the trip
 *      .stat-card__num are plain text, so this owns them exclusively.
 *   b. Staggered icon pop (.stat-card__icon) + a staggered tick reveal
 *      (.inclus-check, respecting --tick-i order), taking over the CSS
 *      baseline via inline styles (no double-animation).
...
(() => {
  ...
  /* ── a. Stat count-up (trip pages) ─────────────────────────────────
     ...
     own [data-counter] animator (which owns the homepage hero stats). */
  ...
  /* ── Reveal wiring ─────────────────────────────────────────────────
     ...
     (v28 note: the homepage has no .stats-grid / .inclus-col — there this
     file only powers the CTA hover nudge below.) */
  function initReveals() {
    const IO = window.IntersectionObserver;
    const stats = document.querySelectorAll('.stats-grid');
    const cols  = document.querySelectorAll('.inclus-col');
    if (!stats.length && !cols.length) return;
