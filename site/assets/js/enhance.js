...
(() => {
  ...
  /* Read one key straight out of the shared dictionary for the page's current
     language. Deliberately NOT window.alTranslate(): that calls translate(),
     which calls captureBaseline(), which snapshots every [data-i18n] node in
     the document as the FRENCH source. On a server-rendered /ar/ page that
     would file the Arabic text away as French — which is exactly why i18n.js's
     own init() skips translate() on variants. Falls back to the French literal
     if i18n.js has not loaded. */
  function dictText(key, fallback) {
    try {
      const lang = document.documentElement.lang || 'fr';
      const dict = window.alTranslations && window.alTranslations[lang];
      const val = key.split('.').reduce((o, k) => (o && k in o) ? o[k] : null, dict);
  ...
  function initWhatsAppFAB() {
    ...
    fab.target = '_blank';
    fab.rel = 'noopener';
    const label = dictText('nav.whatsapp_label', 'Discuter sur WhatsApp');
    fab.setAttribute('aria-label', label);
    fab.title = label;
    // Bound so a later in-place language switch updates them too.
    fab.setAttribute('data-i18n-aria-label', 'nav.whatsapp_label');
    fab.setAttribute('data-i18n-title', 'nav.whatsapp_label');
    fab.innerHTML = `
      <span class="fab-whatsapp__tooltip" data-i18n="nav.whatsapp_tooltip"></span>
    ...
    fab.querySelector('.fab-whatsapp__tooltip').textContent =
      dictText('nav.whatsapp_tooltip', 'Une question ? Écrivez-nous');
