...
// plainText(): the same helper the faqJsonLd codec uses, so a localised
// answer is normalised exactly like the French one it replaces.
import { plainText } from "./codecs.mjs";
...
function localizeHead(html, { lang, slug, langs, data, globalT }) {
  ...
  // FAQPage: the stored block is French plain text, and the visible FAQ section
  // is already translated. faq[i].kQ / .kA carry the same keys and
  // seo.faqJsonLd[i] is the aligned French, so one lookup localises both --
  // there is no second copy of the answers to keep in step.
  const faqLd = (data.seo && data.seo.faqJsonLd) || [];
  ...
  // BreadcrumbList: the trail is the only place these two labels live, so they
  // take sitewide keys rather than per-trip ones. The third crumb is the trip
  // name -- a proper noun -- and stays put.
  const crumbHome = resolve("nav.crumb_home");
  const crumbTrips = resolve("nav.crumb_trips");
  if (crumbHome) html = replaceJsonField(html, "name", "Accueil", crumbHome);
  if (crumbTrips) html = replaceJsonField(html, "name", "Voyages", crumbTrips);

  return html;
}

// ' data-i18n="istFaqQ1"' -> 'istFaqQ1'; also accepts the -html variant.
function keyOf(attr) {
  const m = String(attr || "").match(/data-i18n(?:-html)?="([^"]+)"/);
  return m ? m[1] : null;
}
