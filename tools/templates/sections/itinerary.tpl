...
{{#itinerary.days}}      <div class="tl-day tl-day--{{=i%2===0?"right":"left"}}{{?.active}} active{{/?}}"{{?.wrapperAos}} data-aos="{{=i%2===0?"fade-left":"fade-right"}}" data-aos-delay="{{=i*80}}" data-aos-duration="550"{{/?}}><div class="tl-node"{{.kNode}}>{{.node}}</div><div class="tl-content" data-aos="{{=i%2===0?"fade-left":"fade-right"}}" data-aos-delay="{{=i*80}}" data-aos-duration="550"><p class="tl-day-label"{{.kLabel}}>{{.dayLabel}}</p><h4 class="tl-title"{{.kTitle}}>{{.title}}</h4><p class="tl-activities"{{.kAct}}>{{.activities}}</p>{{?.tags}}<div class="tl-tags">{{#.tags}}<span class="tl-tag"{{.k}}>{{.t}}</span>{{/.tags}}</div>{{/?}}</div></div>
{{/itinerary.days}}    </div>
  </div>
</section>
<!-- ── ITINERARY MAP ───────────────────────────────────────────── -->
