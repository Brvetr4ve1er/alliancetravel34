<section class="hotels section" id="hotels"{{k.hotelsSection}}>
  <div class="container">
    <div class="section-head">
      <span class="phase-marker"><span class="phase-marker__num">2</span><span class="phase-marker__label"{{k.hotelsPhase}}>{{hotelsSection.phaseLabel}}</span></span>{{?hotelsSection.eyebrow}}
      <p class="section-head__eyebrow" data-aos="fade-up" data-aos-duration="400"{{k.hotelsEyebrow}}>{{hotelsSection.eyebrow}}</p>{{/?}}
...
{{#hotels}}      <article class="hotel-card" data-aos="fade-up" data-aos-delay="{{.aosDelay:int}}" data-aos-duration="550"{{.kRole}} data-hotel-id="{{.calcId}}" data-tier="{{.tier}}" tabindex="0"{{.kAria}}>
        <div class="hotel-card__img">
          <img class="hotel-card__photo" src="{{.image}}" alt="{{.alt}}" loading="lazy" width="800" height="600" decoding="async"/>
          <span class="hotel-card__ribbon {{.ribbonClass}}"{{.kRibbon}}>{{.ribbon}}</span>
        </div>
